...
const mapItemsToNotionCard = (items = []) => {
  return items.map(item => ({
    title: item.title,
    author: item.author,
    date: Object(date_fns__WEBPACK_IMPORTED_MODULE_2__["distanceInWords"])(new Date(), new Date(item.published)),
    url: item.originId
  }));
};
...
const getItems = async (event, context, callback) => {
  ...
  const res = await node_fetch__WEBPACK_IMPORTED_MODULE_1___default()(getItemsUrl, {
    method: 'get',
    headers: {
